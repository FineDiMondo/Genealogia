...
let rc = run(process.execPath, [path.join('tools', 'test', 'run_js_tests.mjs')]);
if (rc !== 0) process.exit(rc);

console.log('== STEP 2: DB migration tests ==');
rc = run('python', [path.join('tools', 'test', 'test_db_migration.py')], { shell: true });
if (rc !== 0) process.exit(rc);

console.log('== STEP 3: DB trigger integrity tests ==');
rc = run('python', [path.join('tests', 'schema', 'test_integrity_triggers.py')], { shell: true });
if (rc !== 0) process.exit(rc);
...
rc = run(psExe, ['-ExecutionPolicy', 'Bypass', '-File', path.join('cobol', 'test.ps1')]);
process.exit(rc);
